...
export default async function VendorWholesaleOrdersPage() {
  ...
  const orders = (await getOrdersForVendor(ctx.vendor.slug)).filter(
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-white/50">
          Bulk B2B orders containing your products. Fulfillment is tracked
          separately from retail packing.
        </p>
      </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
                <div>
                  ...
                  <div className="text-xs text-white/40">
                    {o.createdAt.slice(0, 10)} · ship to {o.address.city}, {o.address.state} {o.address.pincode}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-semibold text-white">{formatINR(o.vendorSubtotal)}</span>
              ...
              <ul className="mt-3 space-y-1.5 text-sm">
                {o.items.map((it) => (
                  <li key={it.slug} className="flex justify-between text-white/70">
                    <span>{it.name} <span className="text-white/40">× {it.qty}</span></span>
